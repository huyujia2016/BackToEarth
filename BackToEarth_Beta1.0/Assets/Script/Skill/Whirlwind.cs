...
{
    public static Whirlwind _instance;
    ...
    public override void Use()
    {
        if (tina.CurrentEnergy >= energycost)
        {
            if (IsNotInCoolDown)
            {
                SoundManager._instance.Play("whirlwind", this.GetComponent<AudioSource>());
                tina.ChangeEnergy(-energycost);
                if (tina.Movedirection == MoveDirection.Left)
                {
                    anim.SetTrigger("left_whirlwind");
                }
                else
                {
                    anim.SetTrigger("right_whirlwind");
                }
                base.Use();
            }
        }
    ...
    //0 攻击范围：0为前方，1为周围
    //1音效
    public void WhirlwindEvent(string args)
    {
        ...
        {
            go.SendMessage("TakeDamage", damage+DataSet.Instance().SkillDamageAdditional , SendMessageOptions.DontRequireReceiver);
        }
    }
}
